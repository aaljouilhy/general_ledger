...
def get_result(filters, account_details):
	...
	# Merging 'Remarks' and 'Reference' fields
	for row in data:
		reference = row.get('reference', '')  # Assuming 'reference' is a field
		remarks = row.get('remarks', '')
		row['remarks'] = f"Reference: {reference}; Remarks: {remarks}"
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters.get("show_remarks"):
		# entry['remarks'] = f"{entry['remarks']} - {frappe.format_value(entry['debit'], {'fieldtype': 'Currency'})}"

		if remarks_length := frappe.db.get_single_value(
			"Accounts Settings", "general_ledger_remarks_length"
		):
			# data['combined_field'] = f"{data.get('remarks', '')} - {data.get('debit', 0)}"
			# entry['remarks'] = f"{entry['remarks']} - {frappe.format_value(entry['debit'], {'fieldtype': 'Currency'})}"

			# columns.append({'label': _('Remarks & Debit'), 'fieldname': 'remarks', 'fieldtype': 'Data', 'width': 300})
    ...
    # entry['remarks'] = "{} - {}".format(entry['remarks'], frappe.format_value(entry['debit'], {'fieldtype': 'Currency'}))
...
# })
	...
	if filters.get("show_remarks"):
		...
		data = [
    # {'reference': 'Ref001', 'remarks': 'Sample remark 1'},
    # {'reference': 'Ref002', 'remarks': 'Sample remark 2'},
